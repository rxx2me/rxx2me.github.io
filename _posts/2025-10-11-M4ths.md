...
<ol>
  <li>1) Parse given n and ct.</li>
<li>2) Let A = ceil(sqrt(n)).</li>
<li>3) Work mod n with f(x) = (A + x)^2 − n. For the correct small x0, f(x0) ≡ 0 (mod n).</li>
<li>4) Use a lattice/small-root routine to recover x0 with bound ~n^(1/4).</li>
<li>5) Set a = A + x0, b^2 = a^2 − n; then p = a − b, q = a + b.</li>
<li>6) Compute φ(n) = (p−1)(q−1), d = e^(-1) mod φ(n).</li>
<li>7) Decrypt m = ct^d mod n → extract flag.</li>
</ol>
